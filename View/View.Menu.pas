...
procedure TViewMenu.btnTablesClick(Sender: TObject);
var
  db: TCtrlTables;
begin
...
procedure TViewMenu.FormClose(Sender: TObject; var Action: TCloseAction);
begin
  TThread.Queue(nil,
  procedure
...
procedure TViewMenu.ScrollBoxMouseWheel(Sender: TObject; Shift: TShiftState;
  WheelDelta: Integer; MousePos: TPoint; var Handled: Boolean);
begin
...
procedure TViewMenu.btnPreviousPageClick(Sender: TObject);
begin
  edtPageNumber.Text := IntToStr(StrToInt(edtPageNumber.Text) - 1);
  CreateTableCards();
end;

procedure TViewMenu.btnFirstPageClick(Sender: TObject);
begin
  edtPageNumber.Text := '1';
  CreateTableCards();
end;

procedure TViewMenu.btnLastPageClick(Sender: TObject);
begin
  edtPageNumber.Text := edtPagesCount.Text;
  CreateTableCards();
end;

procedure TViewMenu.btnNextPageClick(Sender: TObject);
begin
  edtPageNumber.Text := IntToStr(StrToInt(edtPageNumber.Text) + 1);
  CreateTableCards();
end;

procedure TViewMenu.CreateTableCards();
var
...
procedure TViewMenu.ClearTables;
var
  table: TDBTable;
begin
